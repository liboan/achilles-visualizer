    ...
    */
    ...
    function initFeatureWindows () { //reads data and adds in feature windows accordingly
        ...
        moreInfo.append("div")
            ...
            .text(function (d) {
                if (d.geneSymbol || d.description) return d.geneSymbol + " " + d.description;
            });
        ...
        //     .append("a")
        //     .text(function (d) {
        //         if (d.geneSymbol) return "TumorPortal entry for " + d.geneSymbol;
        //     })
        //     .attr("target", "blank")
        //     .attr("href", function (d) {
        //         if (d.geneSymbol) return "http://www.tumorportal.org/view?geneSymbol=" + d.geneSymbol;
        //     });
        ...
        var predictionScatters = detailDivs.append("svg")
            ...
            .attr("class", function (d, i) {
                if (data.features[i].varType === "disc") return "box"; //if it's a mutation, it's a box plot
                else return "scatter";
            })
            .attr("id", function (d, i) {
                return "f" + i + "p";
    ...
    function updateGraph(id) { //id = string or int used to access an svg graph. "target" for target/prediction graph, an int for feature
        ...
        if (id !== "target") {
            if (zScoreState && !(data.features[id].varType === "disc")) {
        ...
        if (id === "target" || !(data.features[id].varType === "disc")) { //only add z-score label & values on y-axis if not mutation
        ...
        if (id !== "target") {    
            if (data.features[id].varType === "disc") { //if it's a mutation, draw box plot instead
                updateBoxPlot(id, "p");
                updateBoxPlot(id, "t");
            }
            else {
    ...
    function updateGraphTooltip(graph, xPos) { //Updates the main graph tooltip whenever mouse position shifts, also darkens the appropriate rect
        ...
        if (graph === "target") {
            ...
            if (index >= 0) {
                ...
                if (data.features[graph].varType === "disc") line3 = "";
        ...
        if (graph !== "target" && data.features[graph].varType === "disc") { //add mutations to tooltip if it's a mut feature
